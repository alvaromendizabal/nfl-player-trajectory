...
def model_source(root: Path, model: str, weights: Path) -> tuple[str, list[Path]]:
    """Build the dependency-free numerical cell and list every source/model dependency."""
    ...
    notebook = nbformat.v4.new_notebook(
        cells=[
            nbformat.v4.new_markdown_cell(
                ...
                f"**Model:** `{model}`. Generated by the project owner from verified local weights. "
                "The numerical code and fitted parameters are embedded; no model download is needed. "
