"""Manual, bounded Round 4: offline cached runtime, no Git/AWS/Kaggle writes."""
...
LIMITS={'preflight':120,'smoke':180,'prepare':360,'fit':240,'replay':120,'report':60}
REPORT_NAMES=('preflight.json','smoke.json','preparation.json','summary.json','replay.json','reuse.json','last_command.json','last_error.json')
...
def runtime_command(args):
    ...
    target=dest/'feature_worker.py.lock'
    ...
    return [uv,'run','--frozen','--offline','--no-project','--no-python-downloads',
            '--python',sys.executable,str(dest/'feature_worker.py'),args.command,
            '--kit',str(KIT),'--parent',str(args.parent),'--legacy',str(args.legacy),
            '--repo',str(args.repo),'--out',str(args.out)]


def export_report(out):
    out=Path(out);dest=out/'nfl_feature_round4_report.zip'
...
def safe_paths(args):
    for root in (args.out,args.parent,args.legacy,args.repo,KIT):
        if any(p.is_symlink() for p in (root,*root.parents)):raise ValueError('Symlink workspace roots are not accepted')
    a=args.out.resolve()
    for protected in (args.parent,args.legacy,args.repo,KIT):
        b=protected.resolve()
        if a==b or a.is_relative_to(b) or b.is_relative_to(a):
            raise ValueError('Round 4 results must be separate from prior results and source/data')
...
def main():
    ...
    p.add_argument('--parent',type=Path,default=HOME/'nfl-feature-round3-results')
    p.add_argument('--legacy',type=Path,default=HOME/'nfl_feature_round3')
    p.add_argument('--out',type=Path,default=HOME/'nfl-feature-round4-results')
    ...
    with ExitStack() as stack:
        ...
        except BlockingIOError:raise SystemExit('Another Round 4 stage is running')
        if args.command!='report':
            # Lock an existing prior-run descriptor without changing its bytes.
            old_lock=args.parent/'.run.lock'
            if not old_lock.is_file() or old_lock.is_symlink():raise SystemExit('Expected Round 3 run lock missing; return the report')
            prior=stack.enter_context(old_lock.open('r'))
            try:fcntl.flock(prior,fcntl.LOCK_EX|fcntl.LOCK_NB)
            except BlockingIOError:raise SystemExit('A Round 3 command is active; do not run concurrently')
        try:
            if args.command=='report':
            ...
            if args.command!='report':
                receipt={'utc':datetime.now(timezone.utc).isoformat(),'command':args.command,'exit_code':code,
