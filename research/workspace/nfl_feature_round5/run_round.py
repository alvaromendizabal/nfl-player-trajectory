"""Manual, bounded Round 5: offline cached runtime, no Git/AWS/Kaggle writes."""
...
REPORT_NAMES=('preflight.json','preflight_reuse.json','smoke.json','smoke_reuse.json',
              ...
              'fold_3/futility.json','fold_2/summary.receipt.json','fold_3/summary.receipt.json')
...
def runtime_command(args):
    c=json.loads((KIT/'parent_contract.json').read_text())
    ...
    sources=[p for p in KIT.glob('*.py')]+[KIT/'input_contract.json',KIT/'parent_contract.json']
    ...
    return [uv,'run','--frozen','--offline','--no-project','--no-python-downloads',
            ...
            '--kit',str(KIT),'--round4',str(args.round4),'--round4-kit',str(args.round4_kit),
...
def export_report(out):
    out=Path(out);dest=out/'nfl_feature_round5_report.zip'
...
def safe_paths(args):
    for root in (args.out,args.round4,args.round4_kit,args.round3,args.round3_kit,args.repo,KIT):
        if any(p.is_symlink() for p in (root,*root.parents)):raise ValueError('Symlink workspace roots are not accepted')
    a=args.out.resolve()
    for protected in (args.round4,args.round4_kit,args.round3,args.round3_kit,args.repo,KIT):
        b=protected.resolve()
        if a==b or a.is_relative_to(b) or b.is_relative_to(a):
            raise ValueError('Round 5 results must be separate from prior results and source/data')
...
def main():
    ...
    p.add_argument('--out',type=Path,default=HOME/'nfl-feature-round5-results')
    ...
    with ExitStack() as stack:
        ...
        except BlockingIOError:raise SystemExit('Another Round 5 stage is running')
        if args.command!='report':
            # Lock existing parents without changing their bytes.
            for folder in (args.round3,args.round4):
