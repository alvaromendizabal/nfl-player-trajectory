"""Manual, bounded Round 6: offline cached runtime, no Git/AWS/Kaggle writes."""
...
def runtime_command(args):
    ...
    return [uv,'run','--frozen','--offline','--no-project','--no-python-downloads',
            ...
            '--kit',str(KIT),'--round5',str(args.round5),'--round5-kit',str(args.round5_kit),'--round4',str(args.round4),'--round4-kit',str(args.round4_kit),
...
def export_report(out):
    out=Path(out);dest=out/'nfl_feature_round6_report.zip'
...
def safe_paths(args):
    for root in (args.out,args.round5,args.round5_kit,args.round4,args.round4_kit,args.round3,args.round3_kit,args.repo,KIT):
        if any(p.is_symlink() for p in (root,*root.parents)):raise ValueError('Symlink workspace roots are not accepted')
    a=args.out.resolve()
    for protected in (args.round5,args.round5_kit,args.round4,args.round4_kit,args.round3,args.round3_kit,args.repo,KIT):
        b=protected.resolve()
        if a==b or a.is_relative_to(b) or b.is_relative_to(a):
            raise ValueError('Round 6 results must be separate from prior results and source/data')
...
def main():
    ...
    p.add_argument('--out',type=Path,default=HOME/'nfl-feature-round6-results')
    ...
    with ExitStack() as stack:
        ...
        except BlockingIOError:raise SystemExit('Another Round 6 stage is running')
        if args.command!='report':
            # Lock existing parents without changing their bytes.
            for folder in (args.round3,args.round4,args.round5):
