"""Manual, bounded Round 8: offline cached runtime, no Git/AWS/Kaggle writes."""
...
LIMITS={'preflight':150,'smoke':180,'prepare':360,'runtime':180,'profile':180,
        'train':360,'evaluate':120,'replay':180,'report':60}
REPORT_NAMES=('preflight.json','preflight_reuse.json','smoke.json','smoke_reuse.json',
              'preparation.json','preparation_reuse.json','runtime_receipt.json','profile.json',
              'summary.json','replay.json','last_command.json','last_error.json',
              'models/terminal/complete.json','models/terminal/replay.json','models/terminal/reuse.json',
              'models/history/complete.json','models/history/replay.json','models/history/reuse.json')
...
def runtime_command(args):
    ...
    neural=args.command in ('runtime','profile','train','evaluate','replay')
    entry='neural_worker.py' if neural else 'data_worker.py'
    lock=args.repo/('scripts/motion_supervision.py.lock' if neural else 'scripts/fit_final.py.lock')
    blob=c['neural_lock_git_blob' if neural else 'model_lock_git_blob']
    if not lock.is_file() or lock.is_symlink():raise ValueError('Existing locked runtime missing')
    raw=lock.read_bytes()
    if hashlib.sha1(f'blob {len(raw)}\0'.encode()+raw).hexdigest()!=blob:raise ValueError('Runtime lock identity changed')
    dest=args.out/('neural_runtime' if neural else 'data_runtime');dest.mkdir(parents=True,exist_ok=True)
    for source in list(KIT.glob('*.py'))+[KIT/'input_contract.json']:
        target=dest/source.name;payload=source.read_bytes()
        if target.is_symlink():raise ValueError('Symlink runtime file')
        if target.exists() and target.read_bytes()!=payload:raise ValueError('Frozen runtime source changed; preserve prior work')
        if not target.exists():atomic_bytes(target,payload)
    target=dest/(entry+'.lock')
    ...
    if not uv:raise FileNotFoundError('Existing uv not found')
    opts=[] if args.online else ['--offline']
    cmd=[uv,'run','--frozen',*opts,'--no-project','--no-python-downloads','--python',sys.executable,
         str(dest/entry),args.command,'--kit',str(KIT),'--out',str(args.out)]
    if neural:return cmd+['--arm',args.arm]
    for name in ['repo','round7','round5','round5-kit','round4','round4-kit','round3','round3-kit']:
        cmd+=['--'+name,str(getattr(args,name.replace('-','_')))]
    return cmd


def export_report(out):
    out=Path(out);dest=out/'nfl_feature_round8_report.zip'
...
def safe_paths(args):
    for root in (args.out,args.round7,args.round5,args.round5_kit,args.round4,args.round4_kit,args.round3,args.round3_kit,args.repo,KIT):
        if any(p.is_symlink() for p in (root,*root.parents)):raise ValueError('Symlink workspace roots are not accepted')
    a=args.out.resolve()
    for protected in (args.round7,args.round5,args.round5_kit,args.round4,args.round4_kit,args.round3,args.round3_kit,args.repo,KIT):
        b=protected.resolve()
        if a==b or a.is_relative_to(b) or b.is_relative_to(a):
            raise ValueError('Round 8 results must be separate from prior results and source/data')
...
def main():
    ...
    p.add_argument('--round7',type=Path,default=HOME/'nfl-feature-round7-results')
    ...
    p.add_argument('--out',type=Path,default=HOME/'nfl-feature-round8-results')
    p.add_argument('--arm',choices=['terminal','history'],default='terminal')
    p.add_argument('--online',action='store_true')
    args=p.parse_args()
    if args.online and args.command!='runtime':p.error('--online is allowed only for runtime setup')
    cap=args.seconds if args.seconds is not None else LIMITS[args.command]
    ...
    with ExitStack() as stack:
        ...
        except BlockingIOError:raise SystemExit('Another Round 8 stage is running')
        if args.command!='report':
            # Lock existing parents without changing their bytes.
            for folder in (args.round3,args.round4,args.round5,args.round7):
        ...
        try:
            ...
            if args.command!='report':
                receipt={'utc':datetime.now(timezone.utc).isoformat(),'command':args.command,'arm':args.arm,'exit_code':code,
