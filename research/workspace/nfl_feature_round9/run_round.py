"""Bounded manual signal audit. No train, online, submit, or cloud-write action."""
...
LIMITS={'preflight':90,'feature-smoke':120,'audit':180,'replay':180,'report':60}
REPORT_NAMES=('preflight.json','feature_smoke.json','signal_summary.json','replay.json','last_command.json','last_error.json')


def safe_paths(a):
    for p in (a.out,a.parent,a.parent_kit,a.repo,KIT):
        if not p.is_absolute() or any(x.is_symlink() for x in (p,*p.parents)):
            raise ValueError('Absolute non-symlink paths required')
    for p in (a.parent,a.parent_kit,a.repo,KIT):
...
def export_report(out):
    ...
    path=out/'nfl_feature_round9_report.zip'
    def write(f):
        with zipfile.ZipFile(f,'w',zipfile.ZIP_DEFLATED) as z:
            ...
            z.writestr('CONTENTS.txt','Aggregate diagnostic receipts only. No row predictions, keys, targets, weights or private tensors. No new official RMSE.\n')
    atomic(path,write);print(f'Report: {path}',flush=True);return path


def command(a):
    if a.stage in ('preflight','feature-smoke'):
        worker=KIT/'audit_worker.py';cmd=[sys.executable,str(worker)]
    else:
        c=read_json(KIT/'input_contract.json')
        lock=safe_file(a.repo,'scripts/motion_supervision.py.lock');raw=lock.read_bytes()
        if hashlib.sha1(f'blob {len(raw)}\0'.encode()+raw).hexdigest()!=c['neural_lock_git_blob']:
        ...
        for name in ('sequence_model.py','sequence_features.py','origin_features.py'):
            p=safe_file(a.parent_kit,name)
            if digest(p)!=c['round8_python_hashes'][name]:raise ValueError('Original model source changed')
            sources.append(p)
        ...
        lp=runtime/'audit_worker.py.lock'
        if lp.exists():
        ...
        cmd=[uv,'run','--frozen','--offline','--no-project','--no-python-downloads','--python',sys.executable,str(runtime/'audit_worker.py')]
    cmd+=[a.stage,'--parent',str(a.parent),'--parent-kit',str(a.parent_kit),'--repo',str(a.repo),'--kit',str(KIT),'--out',str(a.out)]
    return cmd
...
def main():
    ...
    p.add_argument('--out',type=Path,default=HOME/'nfl-feature-round9-results')
    ...
    with ExitStack() as stack:
        ...
        except BlockingIOError:raise SystemExit('Another Round 9 stage is active')
        ...
        receipt={'stage':a.stage,'status':'completed' if code==0 else 'stopped','exit_code':code,
                 ...
                 'scientific_fits':0,'optimizer_steps':0,'github_writes':0,'aws_resource_changes':0,
