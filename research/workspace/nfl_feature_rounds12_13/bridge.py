"""Verify the completed Round 11 chain; read parents without modifying them."""
...
def verify_core(a,r,new,v):
    if not r['train']:return
    from new_features import build as original
    prior=v['feature_lookup'][r['parent_path']];p=safe_file(a.audit,prior['path'])
    if digest(p)!=prior['sha256']:raise ValueError('Audited feature bytes changed')
    old=parent_core.arrays(p,('values','valid'));sl=slice(0,6) if a.round_no==12 else slice(6,12)
    if not np.array_equal(old['values'][...,sl],new['values'][...,:6]) or not np.array_equal(old['valid'][...,sl],new['valid'][...,:6]):
        raise ValueError('Published Round 11 six-channel core changed')
    v['protected'][str(p)]=prior['sha256']
...
def features(a,smoke=False,replay=False):
    ...
    for i,r in enumerate(records):
        ...
        if replay or smoke or not dest.is_file():
            p=observed(a,r,v,labels=False);f=build(p,a.round_no);verify_core(a,r,f,v)
            ...
            v['protected'][str(src)]=r['parent_sha256'];verify_core(a,r,f,v)
    ...
    report={'status':'family_smoke_passed' if smoke else 'family_features_ready','round':a.round_no,'signature':v['signature'],
        ...
        'raw_csv_reads':0,'new_model_fits':0,'core_matches_round11_exactly':True}
...
def load_split(a,training):
    ...
    for r in records:
        ...
        p.update(extra=f['values'],extra_valid=f['valid']);total+=sum(x.nbytes for x in p.values())
