...
LIMITS={'preflight':120,'smoke':120,'prepare':360,'runtime':180,'profile':180,'train':360,'evaluate':180,'replay':240,'report':60}
REPORTS=('preflight.json','smoke.json','preparation.json','feature_execution.json','runtime_receipt.json','profile.json','summary.json','replay.json','replay_integrity.json','last_command.json','last_error.json')+tuple(f'models/{arm}/complete.json' for arm in ('mask','core','full'))
...
def main():
    ...
    p.add_argument('--round',dest='round_no',type=int,choices=[12,13],required=True)
    ...
    with ExitStack() as stack:
        ...
        for root in (a.audit,a.parent,a.tensors):
