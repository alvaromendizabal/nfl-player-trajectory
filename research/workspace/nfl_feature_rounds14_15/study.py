...
def protocol(a):
    ...
    p={'study':'motion-anchor-family-v1','round':a.round_no,'config':CONFIG,'environment':environment(),
       ...
       'independence':'Round 14 and 15 candidates do not read each other; common Round 13 motion anchor',
       'review_release_sha256':digest(a.out/'review_release.json')}
...
def profile(a):
    ...
    if (a.out/'profile.json').is_file():
        old=read_json(a.out/'profile.json')
        if old['protocol_signature']!=psig:raise ValueError('Profile lineage changed')
        return old
    ...
    for arm in ARMS:
        ...
        if arm=='full':
            ...
            g=b['node'].grad[...,22:];valid=b['node_valid'][...,22:]
...
def evaluate(a,replay=False):
    ...
    result={'status':'family_study_complete','round':a.round_no,'protocol_signature':psig,'metrics':metrics,
       ...
       'boundary':'Within-round candidate values conditional on shared Round 13 motion inputs; old gain is not confirmed, no promotion or architecture attribution'}
