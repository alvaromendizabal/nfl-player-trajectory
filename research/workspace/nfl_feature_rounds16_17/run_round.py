...
REPORTS=('review_release.json','readiness_status.json','tests_receipt.json','label_readiness.json','preflight.json','smoke.json','preparation.json','feature_execution.json','runtime_receipt.json','profile.json','summary.json','replay.json','replay_integrity.json','last_command.json','last_error.json')+tuple(f'models/{arm}/complete.json' for arm in ('mask','core','full'))
...
def command(a):
    if a.stage=='tests':return [sys.executable,str(KIT/'run_tests.py'),'--out',str(a.out.parent/'nfl-feature-round16-results')]
