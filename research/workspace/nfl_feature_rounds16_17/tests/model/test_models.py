...
def plays(r=16):
    result=[]
    for i in range(1,4):
        p=model_sample(i);e=build(p,r);motion=build_motion(p,13);p.update(extra=e['values'],extra_valid=e['valid'],motion=motion['values'],motion_valid=motion['valid']);result.append(p)
    return result
...
class Models(unittest.TestCase):
    ...
    def test_parameter_count(self):self.assertEqual(sum(p.numel() for p in m.Forecast().parameters()),20402)
    ...
    def test_equal_parameter_shapes_both_rounds(self):
        ...
        for r in (16,17):
            ...
            for arm in ARMS:
                b=m.collate(p,arm,norm);self.assertEqual(b['node'].shape[-1],34);counts.append(sum(x.numel() for x in m.Forecast().parameters()))
    ...
    def test_control_zero_values(self):
        p=plays();b=m.collate(p,'mask',m.normalization(p));self.assertFalse(b['node'][...,22:].any())
    def test_core_retains_exact_first_six(self):
        p=plays();norm=m.normalization(p);a=m.collate(p,'core',norm);b=m.collate(p,'full',norm)
        self.assertTrue(torch.equal(a['node'][...,:28],b['node'][...,:28]));self.assertFalse(a['node'][...,28:].any())
    ...
    def test_nonzero_feature_gradients(self):
        for r in (16,17):
            ...
            self.assertTrue(torch.isfinite(b['node'].grad).all());self.assertTrue(b['node'].grad[...,22:].abs().sum()>0)
