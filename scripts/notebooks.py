...
def local_results(root: Path) -> dict[str, str]:
    ...
    return {name: sha256(folder / name) for name in REPORT_FILES}
...
def execution_signature(root: Path, source: Path) -> str:
    ...
    # Track read-only audit inputs; the demo artifacts are generated by notebook 00.
    inputs.extend(
        path
...
def publish(root: Path, sources: list[Path], expected: dict[str, str], run: Run) -> None:
    ...
    try:
        ...
        raise
    run.event("notebooks_published", notebooks=len(sources), report_files=len(REPORT_FILES))
