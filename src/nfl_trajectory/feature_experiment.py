...
def numerical_sources() -> dict[str, str]:
    return {
        f"{name}.py": sha256(Path(__file__).with_name(f"{name}.py"))
...
def save_week(
    root: Path, pair: dict[str, Any], splits: pd.DataFrame, baseline: dict[str, Any], path: Path
) -> None:
    ...
    truth = pd.read_csv(
        x_path.with_name(x_path.name.replace("input_", "output_")), usecols=KEYS + ["x", "y"]
    )
    ...
    np.savez_compressed(
        buffer,
        keys=truth[KEYS].to_numpy(np.int64),
        truth=truth[["x", "y"]].to_numpy(float),
        ridge=ridge,
        velocity=velocity,
        labels=labels.to_numpy(str),
        bank_keys=bank.state[ENTITY].to_numpy(np.int64),
        bank_state=bank.state[["x", "y", "sign", "num_frames_output"]].to_numpy(float),
        roles=bank.state.player_role.to_numpy(str),
        features=bank.values,
    )
...
def screening(paths: list[Path]) -> tuple[pd.DataFrame, dict[str, list[str]]]:
    ...
    sums = {
        key: np.sum([np.asarray(r[key], dtype=float) for r in rows], axis=0)
    ...
    for name, families in ABLATIONS.items():
        ...
        ranked = eligible.sort_values(
            ["training_residual_correlation", "feature"], ascending=[False, True]
        )
...
def fit_regression(paths: list[Path], names: list[str]) -> dict[str, Any]:
    ...
    total = {
        key: np.sum([np.asarray(r[key], float) for r in records], axis=0)
    ...
    for index in range(len(names)):
        if covariance[index, index] > 1e-10 and (
            not kept or np.abs(normalized[index, kept]).max() < 0.9995
        ):
            kept.append(index)
    ...
    return {
        "features": [names[i] for i in kept],
        "mean": mean[kept].tolist(),
        "scale": scales[kept].tolist(),
...
def predict_residual(
    bank: PlayerFeatures, targets: pd.DataFrame, model: dict[str, Any]
) -> np.ndarray:
...
def evaluate(cache: Path, models: dict[str, Any], output: Path) -> None:
    ...
    if len(targets):
        predictions = {
            "role_ridge": arrays["ridge"][mask],
            "constant_velocity": arrays["velocity"][mask],
        }
...
def report(summary: dict[str, Any], destination: Path) -> None:
    ...
    errors = np.array(
        [
            [r["coordinate_rmse_yards"] - r["rmse_ci95_low"] for r in rows],
    ...
    ax.set_title(
        "Feature-family ablation | frozen temporal validation", loc="left", fontweight="bold"
    )
...
def feature_experiment(
    root: Path, run: Run, checkpoint: Callable[[], object] | None = None
) -> None:
    ...
    with FileLock(str(destination / "pipeline.lock"), timeout=1):
        ...
        if baseline.get("training_games") != training or baseline.get("split_sha256") != sha256(
        ...
        run.event(
            "feature_schema",
            candidate_features=len(feature_catalog()),
            batch_rows=BATCH_ROWS,
            screen_limit=SCREEN_LIMIT,
            selected_limit=SELECT_LIMIT,
        ...
        for i, pair in enumerate(pairs, 1):
            ...
            inputs = [
                source,
                source.with_name(source.name.replace("input_", "output_")),
                baseline_path,
                root / "artifacts/game_splits.csv",
            ]
            stage(
                root,
                f"features-prepare-{name}",
                signature(root, inputs, {"prepare": 1}),
                [cache],
                partial(save_week, root, pair, splits, baseline, cache),
                run,
            )
            ...
            stage(
                ...
                signature(root, [cache], {"screen": "train_only"}),
                [screen],
                partial(screen_week, cache, screen),
                run,
            )
            caches.append(cache)
            ...
            run.event(
                "feature_progress",
                phase="prepare_and_screen",
                completed_weeks=i,
                total_weeks=len(pairs),
            )
        ...
        for name, columns in selections.items():
            ...
            for i, cache in enumerate(caches, 1):
                ...
                stage(
                    root,
                    f"features-statistics-{name}-{cache.parent.name}",
                    signature(root, [cache], {"features": columns}),
                    [stats],
                    partial(regression_week, cache, columns, stats),
                    run,
                )
                statistics.append(stats)
                run.event(
                    "feature_progress",
                    phase="fit_statistics",
                    model=name,
                    completed_weeks=i,
            ...
            stage(
                root,
                f"features-fit-{name}",
                signature(root, statistics, {"alpha": RIDGE_ALPHA, "features": columns}),
                [model_path],
                partial(save_regression, statistics, columns, model_path),
                run,
            )
            ...
            if checkpoint is not None:
                checkpoint()
            run.event(
                "features_selected", model=name, selected_features=len(models[name]["features"])
            )
        ...
        for i, cache in enumerate(caches, 1):
            ...
            stage(
                root,
                f"features-evaluate-{cache.parent.name}",
                signature(root, [cache, model_path], {"evaluate": 1}),
                [output],
                partial(evaluate, cache, models, output),
                run,
            )
            errors_paths.append(output)
            run.event(
                "feature_progress", phase="evaluate", completed_weeks=i, total_weeks=len(caches)
            )
        ...
        def aggregate() -> None:
            ...
            for row in summary["models"]:
                ...
                row["selected_features"] = (
                    len(models[row["model"]]["features"])
                    if row["model"] in models
                    else (6 if row["model"] == "role_ridge" else 1)
                )
            summary.update(
                {
                    ...
                    "split_sha256": baseline["split_sha256"],
                    "numerical_signature": report_signature,
                    "source_sha256": numerical_sources(),
                    "baseline_sha256": sha256(baseline_path),
                    "family_counts": {
                        str(k): int(v) for k, v in catalog.groupby("family").size().items()
                    },
                    "top_training_associations": catalog.sort_values(
                        "training_residual_correlation", ascending=False
                    )
                    .head(20)
                    .to_dict(orient="records"),
                    "limitation": (
                        "Training-only univariate screening and fixed ridge; "
        ...
        stage(
            ...
            "features-report",
            report_signature,
            [summary_path, figure_path, model_path],
            aggregate,
            run,
        )
        if checkpoint is not None:
