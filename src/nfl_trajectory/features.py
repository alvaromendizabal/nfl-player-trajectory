...
GATE_INPUTS = tuple(f"lag00__{c}" for c in CHANNELS) + tuple(
    f"{side}{slot}__{c}"
    for side, count in NEIGHBOR_GROUPS
...
def feature_catalog() -> pd.DataFrame:
    ...
    for channel in CHANNELS:
        ...
        records += [
            (f"w{w:02d}__{channel}__{stat}", "history_summaries") for w in WINDOWS for stat in STATS
        ]
    ...
    records += [
        ...
        for name in (
            "field_x",
            "field_y",
            "line_of_scrimmage_dx",
            "line_of_scrimmage_present",
            "horizon_seconds",
            "history_seconds",
    ...
    records += [
        (f"{gate}__{channel}", "forecast_interactions")
    ...
    catalog["signal"] = [
        "interaction"
        if any(group in name for group, _ in NEIGHBOR_GROUPS)
        else "landing"
        if any(term in name for term in landing)
        else "motion"
...
def _history(inputs: pd.DataFrame) -> pd.DataFrame:
    ...
    if (
        (
            play[["play_direction", "num_frames_output", "ball_land_x", "ball_land_y"]].nunique(
                dropna=False
            )
            > 1
        )
    ...
    for c in ("s", "a", "dir", "o"):
        v = (
            pd.to_numeric(h[c], errors="raise").to_numpy(float)
            if c in h
    ...
    h["jerk"] = np.nan_to_num(
        _divide(np.hypot(h.ax - previous.ax, h.ay - previous.ay), dt), nan=0.0
    )
...
def _neighbors(row: pd.Series, play: pd.DataFrame) -> list[float]:
    ...
    for team, count in NEIGHBOR_GROUPS:
        if team in ("teammate", "opponent"):
            ...
            candidates = play[
                (same if team == "teammate" else ~same) & play.nfl_id.ne(row.nfl_id)
            ].copy()
        else:
        ...
        for slot in range(count):
            ...
            values.extend(
                [
                    ...
                    dvy,
                    np.sqrt(vv),
                    -dot / max(distance, 1e-8),
                    closest,
                    closest_distance,
                    n.ball_distance,
                    n.ball_distance - row.ball_distance,
                    alignment,
                    (row.release_frame - n.frame_id) / 10.0,
                    1.0,
                ]
            )
        if team in ("teammate", "opponent"):
            values.extend(
                [float((candidates.distance <= radius).sum()) for radius in (2, 5, 10, 20)]
            )
    return values
...
class PlayerFeatures:
    ...
    def matrix(self, targets: pd.DataFrame, columns: list[str] | None = None) -> np.ndarray:
        ...
        if len(chosen) != len(set(chosen)) or not set(chosen).issubset(names):
            raise ValueError(
                "Requested feature names must be unique members of the feature schema."
            )
        ...
        index = targets[ENTITY].merge(
            self.state[ENTITY].assign(_row=np.arange(len(self.state))),
            on=ENTITY,
            how="left",
            sort=False,
...
def build_player_features(
    inputs: pd.DataFrame, entities: pd.DataFrame | None = None
) -> PlayerFeatures:
    ...
    for key, history in h.groupby(ENTITY, sort=False):
        ...
        lag_values = np.stack(
            [
                series[positions[int(frames[-1]) - lag]]
                if int(frames[-1]) - lag in positions
        ...
        summaries = np.stack(
            [
                _summaries(series[frames > frames[-1] - w], frames[frames > frames[-1] - w])
                for w in WINDOWS
            ],
            axis=1,
        )
        values = (
            np.concatenate([lag_values, summaries.reshape(len(CHANNELS), -1)], axis=1)
            .ravel()
            .tolist()
        )
        ...
        values.extend(
            [
                float(row.x),
                float(row.y),
                row.x - los if np.isfinite(los) else 0.0,
                float(np.isfinite(los)),
                row.num_frames_output / 10.0,
                (frames[-1] - frames[0]) / 10.0,
                (row.release_frame - frames[-1]) / 10.0,
            ]
        )
        rows.append(values)
        states.append(
            dict(zip(ENTITY, key, strict=True))
            | {
                "x": float(row.raw_x),
                "y": float(row.raw_y),
                "sign": float(row.sign),
