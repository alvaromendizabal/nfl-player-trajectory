...
def test_exporter_change_invalidates_notebook_checkpoint(project: Path) -> None:
    source = notebook(project)
    (project / "kaggle").mkdir()
    exporter = project / "kaggle/export.py"
    exporter.write_text("# first exporter")
    before = runner.execution_signature(project, source)
    exporter.write_text("# changed exporter")
    assert runner.execution_signature(project, source) != before
