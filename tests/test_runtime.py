...
def test_active_stage_heartbeat_has_utc_stage_and_total_elapsed_time(tmp_path):
    import threading

    with Run(tmp_path, "test", heartbeat_seconds=0.005) as run:
        stage(tmp_path, "work", "timing", [], lambda: threading.Event().wait(0.035), run)
    rows = [json.loads(line) for line in run.log_path.read_text().splitlines()]
    assert all(row["timestamp"].endswith("+00:00") for row in rows)
    assert all(row["total_elapsed_seconds"] >= row["stage_elapsed_seconds"] >= 0 for row in rows)
    assert any(row["event"] == "heartbeat" and row["stage"] == "work" for row in rows)
